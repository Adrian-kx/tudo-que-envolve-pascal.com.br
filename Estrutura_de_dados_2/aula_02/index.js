...
// Função para explorar todos os caminhos possíveis de um nó para outro
function findAllPaths(graph, startNode, endNode, visited = {}, path = [], allPaths = [], visitLimit = 2) {
    path.push(startNode);
    ...
    if (startNode === endNode) {
        ...
        for (let neighbor in graph[startNode]) {
            if (!visited[neighbor] || visited[neighbor] < visitLimit) {
                findAllPaths(graph, neighbor, endNode, visited, path, allPaths, visitLimit);
            }
        }
    }
...
// Função para calcular o custo de um caminho específico
...
// Função para encontrar todas as rotas e seus respectivos custos
...
// Função para encontrar o caminho com o menor custo
function findShortestPath(pathsWithCosts) {
    return pathsWithCosts.reduce((min, current) => (current.cost < min.cost ? current : min), pathsWithCosts[0]);
}
...
// Definindo o nó inicial e o nó de destino
const startNode = "POUSO";
const endNode = "RDS";

// Encontrando todas as rotas e custos
const allPathsAndCosts = getAllPathsAndCosts(graph, startNode, endNode);

// Exibindo todas as possibilidades de caminhos com seus respectivos custos
console.log(`Possibilidades de caminhos de ${startNode} para ${endNode}:`);
allPathsAndCosts.forEach((p, index) => {
    console.log(`Possibilidade ${index + 1} (Custo: ${p.cost.toFixed(2)}): ${p.path.join(' > ')}`);
});

// Encontrando e exibindo o caminho com o menor custo
const shortestPath = findShortestPath(allPathsAndCosts);
console.log(`\nCaminho com menor custo: ${shortestPath.path.join(' > ')} (Custo: ${shortestPath.cost.toFixed(2)})`);
